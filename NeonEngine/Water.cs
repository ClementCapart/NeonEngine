...
namespace MetroEngine
{
	public class Water : Entity
	{
		...
		public Water(World containerWorld, Rectangle area)
            : base(containerWorld, "water")
		{
            entityType = GameObjectType.GameObject;
            this.area = area;
            ...
            emitSplashParticles = Metro.waterSplash;
            ...
            particleTexture = Metro.utils.generateRadialGradient(10);
            metaballTarget = new RenderTarget2D(Metro.graphicsDevice, Metro.ScreenWidth, Metro.ScreenHeight);
            particlesTarget = new RenderTarget2D(Metro.graphicsDevice, Metro.ScreenWidth, Metro.ScreenHeight);
            alphaTest = new AlphaTestEffect(Metro.graphicsDevice);
			alphaTest.ReferenceAlpha = 175;

            var view = Metro.graphicsDevice.Viewport;
		...
		{
            int index = (int)MathHelper.Clamp((int)(x - area.X) / Scale, 0, columns.Length - 1);
		...
		public void DrawToRenderTargets()
		{
            if (emitSplashParticles)
            {
                ...
                {
                    Vector2 p1 = new Vector2((i - 1) * scale, columns[i - 1].Height + 2);
                    Vector2 p2 = new Vector2(i * scale, columns[i].Height + 2);
        ...
        public override void Draw(SpriteBatch spriteBatch)
        {
            ...
            if (emitSplashParticles)
            {
                ...
                spriteBatch.Draw(particlesTarget, new Vector2(area.X, area.Y) - Vector2.One, new Color(0.8f, 0.8f, 1f));
                spriteBatch.Draw(particlesTarget, new Vector2(area.X, area.Y) + Vector2.One, new Color(0f, 0f, 0.2f));
                spriteBatch.Draw(particlesTarget, new Vector2(area.X, area.Y), light);
            }
			...
			for (int i = 1; i < columns.Length; i++)
			{
                ...
                Vector2 halfScreen = new Vector2(Metro.ScreenWidth / 2, Metro.ScreenHeight / 2);

                pb.AddVertex(p1 - containerWorld.camera.Position + halfScreen, light);
                pb.AddVertex(p2 - containerWorld.camera.Position + halfScreen, light);
                pb.AddVertex(p3 - containerWorld.camera.Position + halfScreen, dark);

                pb.AddVertex(p1 - containerWorld.camera.Position + halfScreen, light);
                pb.AddVertex(p3 - containerWorld.camera.Position + halfScreen, dark);
                pb.AddVertex(p4 - containerWorld.camera.Position + halfScreen, dark);
			}
